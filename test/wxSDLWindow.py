...
class wxSDLWindow(wx.Frame):
	def __init__(self, parent, id, title='SDL window', **options):
		...
		self.FrameSize = options['size']
		self.SurfaceSize = self.FrameSize
	...
	def OnIdle(self, event):
		...
		if self.__needsDrawing:
			self.draw()
		#things that need running continiously
		self.process()
	...
	def processHandling(self):
		raise NotImplementedError('please define a proces method')
